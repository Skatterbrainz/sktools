...
function Get-SKDbValueLink {
    ...
    if (![string]::IsNullOrEmpty($Value)) {
        switch ($ColumnName) {
            ...
            'CollectionID' {
                $output = "<a href=`"cmcollection.ps1?f=collectionid&v=$Value&t=$Script:CollectionType&n=&tab=general`" title=`"Details`">$Value</a>"
                break;
            }
            'CollectionName' {
...
function New-SkMenuTabSet2 {
    ...
    foreach ($tab in $tabs) {
        $xlink = "$baselink`?f=$($Script:SearchField)&v=$($Script:SearchValue)&x=$($Script:SearchType)&s=$($Script:SortField)&so=$($Script:SortOrder)&n=$($Script:CustomName)&tab=$tab"
