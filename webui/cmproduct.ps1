...
$SortField   = Get-SkPageParam -TagName 's' -Default 'productname'
...
try {
    ...
    if ($rs.BOF -and $rs.EOF) {
        ...
        while (!$rs.EOF) {
            ...
            $xx = "<a href=`"cminstalls.ps1?pn=$pn&pv=$pv`" title=`"Show Installations`">$qx</a>"
            $px = "<a href=`"cmproduct.ps1?pn=$pn&pv=$pv`" title=`"Details for $pn`">$pn</a>"
...
$content += Write-SkDetailView -PageRef "cmproducts.ps1" -Mode $Detailed
