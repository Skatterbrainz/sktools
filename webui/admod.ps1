...
if ([string]::IsNullOrEmpty($username) -and [string]::IsNullOrEmpty($groupname)) {
    ...
    elseIf (![string]::IsNullOrEmpty($username)) {
        ...
        $list = ""
        [array]$groupsAll = Get-SkAdGroups | Sort-Object Name | Select -ExpandProperty Name 
        [array]$groupsUsr = Get-SkAdUserGroups -UserName $username | Sort-Object Name | Select -ExpandProperty Name 
